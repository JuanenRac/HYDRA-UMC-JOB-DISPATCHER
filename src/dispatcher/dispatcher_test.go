...
func TestJob_MutatingReturnedDependsOnDoesNotCorruptInternalState(t *testing.T) {
	e := NewEngine()
	if err := e.AddJob(Job{ID: "pick"}); err != nil {
		t.Fatalf("AddJob pick: %v", err)
	}
	if err := e.AddJob(Job{ID: "place", DependsOn: []string{"pick"}}); err != nil {
		t.Fatalf("AddJob place: %v", err)
	}

	got, ok := e.Job("place")
	if !ok {
		t.Fatalf("Job(place) not found")
	}
	got.DependsOn[0] = "injected-without-engine-method"

	again, _ := e.Job("place")
	if again.DependsOn[0] != "pick" {
		t.Fatalf("internal DependsOn = %q after external mutation, want unaffected %q", again.DependsOn[0], "pick")
	}
}

// Same real bug, the input side: mutating the []string a caller passed
// into AddJob, after the call returns, must not retroactively change the
// job's stored dependency graph.
func TestAddJob_MutatingCallersDependsOnSliceAfterSubmitDoesNotCorruptInternalState(t *testing.T) {
	e := NewEngine()
	if err := e.AddJob(Job{ID: "pick"}); err != nil {
		t.Fatalf("AddJob pick: %v", err)
	}
	deps := []string{"pick"}
	if err := e.AddJob(Job{ID: "place", DependsOn: deps}); err != nil {
		t.Fatalf("AddJob place: %v", err)
	}

	deps[0] = "injected-without-engine-method"

	place, _ := e.Job("place")
	if place.DependsOn[0] != "pick" {
		t.Fatalf("stored DependsOn = %q after external mutation of the caller's own slice, want unaffected %q", place.DependsOn[0], "pick")
	}
}

// Jobs() must return the same real, independent copy as Job() - a
// mutation of one entry in the returned slice must not corrupt Engine's
// internal state either.
func TestJobs_MutatingReturnedDependsOnDoesNotCorruptInternalState(t *testing.T) {
	e := NewEngine()
	if err := e.AddJob(Job{ID: "pick"}); err != nil {
		t.Fatalf("AddJob pick: %v", err)
	}
	...
	for _, j := range e.Jobs() {
		if j.ID == "place" {
			j.DependsOn[0] = "injected-without-engine-method"
		}
	}

	place, _ := e.Job("place")
	if place.DependsOn[0] != "pick" {
		t.Fatalf("internal DependsOn = %q after mutating a Jobs() entry, want unaffected %q", place.DependsOn[0], "pick")
	}
}

// A retried job's returned DependsOn must be independent of the internal
// state too - same real gap, SubmitJob's own retry return path.
func TestSubmitJob_RetriedJobReturnsIndependentDependsOnCopy(t *testing.T) {
	e := NewEngine()
	if err := e.AddJob(Job{ID: "pick"}); err != nil {
		t.Fatalf("AddJob pick: %v", err)
	}
	...
	if _, _, err := e.SubmitJob(Job{ID: "flaky", DedupKey: "flaky-key", DependsOn: []string{"pick"}}); err != nil {
	...
	retried, _, err := e.SubmitJob(Job{ID: "flaky-retry", DedupKey: "flaky-key", DependsOn: []string{"pick"}})
	if err != nil {
		t.Fatalf("retry submit: %v", err)
	}
	retried.DependsOn[0] = "injected-without-engine-method"

	again, _ := e.Job("flaky")
	if again.DependsOn[0] != "pick" {
		t.Fatalf("internal DependsOn = %q after mutating the retried return value, want unaffected %q", again.DependsOn[0], "pick")
	}
}
...
// active assignment.
func TestUpsertRobot_HeartbeatCannotReclaimARobotWithAnActiveAssignment(t *testing.T) {
	e := NewEngine()
	e.UpsertRobot(Robot{ID: "robot-a", Available: true})
	...
	// A stale/duplicate heartbeat arrives while robot-a is still working
	// job-1 - before the fix, this silently reopened robot-a for a second
	// assignment.
	e.UpsertRobot(Robot{ID: "robot-a", Available: true})

	second := e.DispatchOnce()
	if len(second) != 0 {
		t.Fatalf("second dispatch = %+v, want no assignment - robot-a is still busy with %q", second, first[0].JobID)
	}

	robots := e.Robots()
	if len(robots) != 1 || robots[0].Available {
		t.Fatalf("robots = %+v, want robot-a to still report Available=false while its job is in flight", robots)
	}
	...
	if len(third) != 1 {
		t.Fatalf("third dispatch = %+v, want the remaining job now assigned", third)
	}
}

// A heartbeat declaring Available=false for a robot with no active
// assignment must still work normally - the fix only guards the
// currently-reserved case, never a genuinely idle robot.
func TestUpsertRobot_HeartbeatStillControlsAvailabilityForAnUnreservedRobot(t *testing.T) {
	e := NewEngine()
	e.UpsertRobot(Robot{ID: "robot-a", Available: true})

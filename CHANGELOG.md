...
All notable work on **HYDRA-UMC-JOB-DISPATCHER** is summarized here, newest first.
...
- **JOB-01 (found in an ecosystem-wide software-improvements audit, P1):**
  ...
  silently rewrote Engine's real dependency graph - bypassing the mutex
...
- **JOB-02 (found in the same audit, P0):** `Robot.Available` doubled as
  ...
  already assigned a job to (e.g. a heartbeat racing that assignment),
  letting a second `DispatchOnce` call hand a different job to the same
  physically-busy robot. `UpsertRobot` now checks the scheduler's own real
  reservation (any job still `StatusAssigned` to that robot) before ever
  letting a heartbeat's `Available` value take effect - a heartbeat can
  still freely report `Available=false` for a genuinely idle robot, it
  just can no longer flip a reserved one back to available.
- 7 new regression tests (dispatcher package) reproducing each finding's
  own exact scenario (external mutation of a returned/submitted
  `DependsOn`; two jobs, one robot, a heartbeat in between - exactly one
  active assignment). `go test -mod=readonly ./...`: all passing. `-race`
  could not be run in this environment (no cgo/C toolchain available, and
  this repo's own CI does not run it either); every new/changed field
  access still happens exclusively under `e.mu`, the same discipline the
  rest of this file already follows.
- Full lease/epoch-based reservation, restart/expiry reconciliation and
  crash-replay tests remain real, deferred future work - this closes the
  concrete, reproduced double-assignment race, not the full aspirational
...
- Found in an ecosystem-wide software-improvements audit: the README's
  ...
  from day one specifically so a real backing store could be added
  later without changing every caller. This is that store, real and
  verified end-to-end, not a stub.
- **`src/dispatcher/dispatcher.go`** - new `Store` interface
  (`SaveJob`/`SaveRobot`/`LoadAll`) and `JobRecord` (a job's public
  fields plus its internal sequence number, needed to restore FIFO
  order across a restart). `NewEngineWithStore(store)` loads existing
  state first, then every mutation writes through: a brand-new job's
  first-ever persist is a real, returned error (nothing is left
  half-written in memory if it fails); every other transition
  (assignment, completion, retry, the cascading Blocked/Unreachable
  re-evaluation) persists too, but a failure there is recorded via the
  new `LastPersistError()` instead of undoing an already-real, already-
  physical event (a robot that just finished a job is never told "sorry,
  undone" because a database write hiccuped afterward). `NewEngine()`'s
  existing purely in-memory behavior is completely unchanged - every one
  of the ~30 existing call sites across this package's own tests needed
  zero changes.
- **`src/sqlitestore`** (new package) - the real, shipped `Store`
  implementation: `modernc.org/sqlite` (pure Go, no CGO), this repo's
  first external dependency - deliberate, so `GOOS=linux GOARCH=arm64 go
  build` for the CM5 keeps working from any dev machine without a
  matching C cross-toolchain. One real file (or `:memory:` for tests) is
  the whole deployment story.
- **`src/api/api.go`** - new `GET /health`, reporting
  `persistenceHealthy`/`lastPersistError` from `LastPersistError()` - a
  degraded store is now a real, observable signal, not a silent one.
- **`main.go`** - new `-db PATH` flag: unset, this runs exactly as
  before (purely in-memory); set, the mission queue survives this
  process restarting.
- Verified for real, not just via unit tests: built the actual binary,
  ran it against a real SQLite file, submitted a job, registered a
  robot, dispatched it - then killed the process and started a genuinely
  new one pointed at the same file. `GET /jobs`/`GET /robots` from the
  second process reported the exact same in-flight state (job
  `assigned` to the robot, robot `unavailable`) the first process had
  reached, and `GET /health` reported healthy.
- `go vet`/`go build`/`go test ./...` all pass (dispatcher: 20 tests
  including a real cross-process restart test against `sqlitestore`;
  sqlitestore: 6 tests including its own real reopen-the-same-file test;
  api: 11 tests including the new `/health` coverage).

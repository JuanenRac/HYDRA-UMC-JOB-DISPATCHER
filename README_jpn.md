...
**正直な現状確認 - 今日実際に動くもの:** スケジューリングエンジン(`src/dispatcher/dispatcher.go`)——優先度順キュー、ツールを考慮したルーティング、複数段階の依存関係追跡、そして `DedupKey` による冪等な投入——およびそれをラップする単純な JSON/HTTP API(`src/api/api.go`)は本物であり、テスト済みである(`src/dispatcher`、`src/api`、`src/sqlitestore` 全体で 43 件のテストが成功、`go test ./...`)。オプションの SQLite 永続化(`src/sqlitestore/sqlitestore.go`、CGO 不使用の純粋な Go)は、実際に強制終了して再起動した本物のプロセスに対してエンドツーエンドで検証されており、単に分離された単体テストではない。既知かつ文書化されているギャップ: ツールを考慮したルーティングは、ロボット自身の `POST /robots` 登録が申告する内容に対して `RequiredTool`/`Robot.Tool` の完全一致文字列比較でチェックしているだけであり、まだ CAN 経由で実際の URTC と通信してツールヘッドが物理的に装着されているかどうかを確認していない。そのため、自分のツールについて嘘をつくロボット(あるいはヘッドが外れてしまったロボット)は、今日の時点では正直なロボットと区別がつかない。ロードマップの 4 つのフェーズ(TSN 同期、3D 経路計画、ディスパッチ最適化、AI 駆動の所要時間推定)は、今のところ裏付けとなるコードが一切ない、将来に向けた願望である。これまでに何が実際に出荷されたかは `CHANGELOG.md` を参照。
...
* 📡 **永続性：** 実際に動作するオプションの SQLite バックエンドによるミッション状態管理（`-db PATH`、`src/sqlitestore`、純粋な Go - CGO 不要） - 実際のプロセス再起動を経てもキューは生き残ることをエンドツーエンドで検証済み。未設定の場合はメモリ上のみで動作します。`GET /health` は劣化したストアを黙って失敗させず報告します。詳細は [`docs/API.md`](docs/API.md#persistence) を参照。
...
* **スケジューラも、そして今では永続化も、どちらも本物です。** `src/dispatcher` は README の "DISPATCHER FLOW" 図が説明する実際のアルゴリズムを実装しています：優先度順に並んだグローバルキュー、工具を意識したルーティング、さらに多段階依存関係（`DependsOn` 内のすべてのジョブが `done` に達するまで、ジョブは `blocked` のまま）。`Engine` の状態は、後で実際のストアが各呼び出し側を変えることなくこれらのメソッドの背後にあるものを置き換えられるよう、もともとエクスポートされたメソッドの背後に保持されていました——`NewEngineWithStore`（`src/sqlitestore`）がそのストアです：オプション、純粋な Go による SQLite、実際に終了・再起動させたプロセスに対してエンドツーエンドで検証済み。詳細は [`docs/API.md`](docs/API.md#persistence) を参照。
